    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className={`w-full py-3 px-6 ${c.btnBg} text-white font-semibold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg active:scale-95 text-base`}
                >
                    Got it
                </button>
            </div>
...
export default function UAEForm({ countries }: Props) {
    ...
    const onSubmit = async (data: RegistrationForm) => {
        ...
        try {
            setLoading(true);
            const country_code = countries.find((c) => c.value === "uae")?.code || "3";
            const payload = { ...data, country_code: String(country_code) };
            await submitFormData(data);
            setDialog({ type: "success", message: "Registration completed successfully! Welcome aboard." });
        } catch (error: any) {
            setDialog({ type: "error", message: error?.message || "Registration failed. Please try again." });
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-5">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <Section title="Primary Details">
                        ...
                        <InputField label="Student Mobile" placeholder="Enter mobile"
                            registration={register("studentMobile", {
                                ...
                                pattern: { value: /^[0-9]{9,10}$/, message: "Student Mobile must be 10 digits" },
                            })}
                    ...
                    <Section title="Parent Details">
                        ...
                        <InputField label="Parent Mobile" required placeholder="Enter parent mobile"
                            registration={register("parentMobile", {
                                ...
                                pattern: { value: /^[0-9]{9,10}$/, message: "Parent Mobile must be 9 or 10 digits" },
                            })}
                        ...
                        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                            ...
                            {/* Send OTP + OTP input side by side */}
                      ...
                      ${cooldown > 0
                                    ...
                                    {emailVerified && (
                                        <span className="flex items-center gap-1 text-green-600 font-semibold text-sm bg-green-50 px-3 py-1.5 rounded-full border border-green-200">
                                ...
                                {otpModalOpen && !emailVerified && (
                                    <div className="flex items-center gap-2 mt-1">
                                        <input
                                            ...
                                            className="border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 px-3 py-2 rounded-lg w-44 text-sm outline-none transition-all"
                                        />
                                        <button
                                            type="button"
                                            onClick={verifyOtp}
                                            disabled={verifyLoading}
                                            className="flex items-center gap-1.5 px-4 py-2 bg-green-600 hover:bg-green-700 disabled:opacity-60 text-white font-medium text-sm rounded-lg transition-all duration-200 shadow-sm active:scale-95"
                                        >
                                            {verifyLoading ? (
                                                <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    ...
                    <div className="pt-4 text-center">
                        <Button type="submit" variant="primary" loading={loading} loadingText="Submitting..." className="px-8 py-3">
                            Submit Registration
                        </Button>
                    </div>
                </form>
